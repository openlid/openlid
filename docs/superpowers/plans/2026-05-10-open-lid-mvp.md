...
> **Critical:** This is a direct port of `Sources/Modafinil/LidMonitor.swift`. The IOKit constants (`errSystem(0x38) | errSub(13) | 0x100`) and the `IOServiceAddInterestNotification` callback shape must match.
    ...
    D: DisplayController + Send + Sync + 'static,
{
    ...
    fn on_lid_change(self: &Arc<Self>, new_lid: LidState) {
        ...
        self.reconcile();
        if was_closing {
            // Mirror Modafinil: force display sleep on lid-close (no ext display)
...
closed, while letting the display turn off. Inspired by [Modafinil](https://github.com/narcotic-sh/modafinil) —
...
**Spec coverage check:**
- ✅ Goal 1 (modafinil feature parity): Tasks 17 (lid monitor) + 18 (display) + Tasks 10-15 (helper toggling disablesleep)
