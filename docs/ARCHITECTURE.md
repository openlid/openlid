...
OpenLid.app/Contents/MacOS/open-lid-helper        (signed, root via launchd)
...
one pure function in `open-lid-core::state`:
...
The Tabler Icons paths are MIT-licensed and used in two places: the menu
bar status item and the app icon. The app icon is generated by
`scripts/generate-icon.sh` from an embedded SVG, rendered via macOS
QuickLook + `sips` + `iconutil` — entirely with built-in macOS tooling.

## Future cross-platform plans

The `open-lid-core` traits already represent the cross-platform shape:
